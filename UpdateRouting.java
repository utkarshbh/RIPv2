				...
				for (int i = 0; i < neighList.size(); i++) {
						...
						if(Process.neighbourList.containsKey(fromIpNtwPrefix) && !(neighList.get(i).getNextHop().equalsIgnoreCase(localNetwPrefix))){
							if(availablePath == false){
								//System.out.println("1. Destination is: "+neighList.get(i).getDestIpAddr());
								...
								if(Process.routingTable.get(indexVal).getDistance() > (neighList.get(i).getDistance() + Process.neighbourList.get(fromIpNtwPrefix))){
									//System.out.println("2. Destination is: "+neighList.get(i).getDestIpAddr()+" oldval: " + Process.routingTable.get(indexVal).getDistance()+" newval: "+neighList.get(i).getDistance() + " + " + Process.neighbourList.get(fromIpNtwPrefix));
